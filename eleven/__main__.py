...
from eleven import dispatcher, updater, TOKEN, WEBHOOK, OWNER_ID, DONATION_LINK, CERT_PATH, PORT, URL, LOGGER, spamcheck
# needed to dynamically load modules
# NOTE: Module order is not guaranteed, specify that in the config file!
from eleven.modules import ALL_MODULES
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("eleven.modules." + module_name)
...
def start(update, context):
    if update.effective_chat.type == "private":
        ...
        if len(args) >= 1:
            ...
            update.effective_message.reply_photo(HELP_PANEL_STRING.ELEVEN_IMG, PM_START_TEXT, reply_markup=InlineKeyboardMarkup(buttons), parse_mode=ParseMode.MARKDOWN)
    else:
    ...
    try:
        if mod_match:
            ...
            try:
                ...
                set_button = []
            set_button.append([InlineKeyboardButton(text=tl(query.message, "Back"),
            ...
            chat = context.bot.get_chat(chat_id)
            query.message.edit_text(text=tl(update.effective_message, "Hi there! There are quite a few settings for {} - go ahead and pick what you're interested in.").format(chat.title),
                                  reply_markup=InlineKeyboardMarkup(
            ...
            chat = context.bot.get_chat(chat_id)
            query.message.edit_text(text=tl(update.effective_message, "Hi there! There are quite a few settings for {} - go ahead and pick what you're interested in.").format(escape_markdown(chat.title)),
                                  parse_mode=ParseMode.MARKDOWN,
...
def donate(update, context):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 834309762 and DONATION_LINK:
            update.effective_message.reply_text(tl(update.effective_message, "You can also donate to the person currently running me [here]({})").format(DONATION_LINK),
                                                parse_mode=ParseMode.MARKDOWN)
        ...
        try:
            ...
            update.effective_message.reply_text(tl(update.effective_message, "I've PM'ed you about donating to my creator!"))
        except Unauthorized:
            update.effective_message.reply_text(tl(update.effective_message, "Contact me in PM first to get donation information."))
