...
def markdown_parser(txt: str, entities: Dict[MessageEntity, str] = None, offset: int = 0) -> str:
    ...
    for ent, ent_text in entities.items():
        ...
        if ent.type in ("bold", "italic", "code", "url", "text_link"):
...
def extract_time(message, time_val):
    if any(time_val.endswith(unit) for unit in ('m', 'h', 'd')):
        ...
        if not time_num.isdigit():
            message.reply_text("The amount of time is not determined.")
            return ""
        ...
        message.reply_text("Invalid time type specified. Expected m, h, or d, obtained: {}".format(time_val[-1]))
        return ""
