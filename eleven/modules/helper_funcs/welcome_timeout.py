...
from eleven import dispatcher, updater, spamcheck, IS_DEBUG
...
from eleven.modules.helper_funcs.string_handling import make_time, extract_time_int
		...
		if int(time.time()) >= int(cht.timeout_int):
			...
			if timeout_mode == 1:
				...
				try:
					...
					pass
					# send_message_raw(chat_id, tl(user_id, "Verification failed!\nut was failed to banned {}: {}").format(mention_markdown(user_id, context.bot.getChatMember(chat_id, user_id).user.first_name), str(err)), parse_mode="markdown")
