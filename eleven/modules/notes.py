...
from eleven import dispatcher, MESSAGE_DUMP, LOGGER, spamcheck, OWNER_ID
...
from eleven.modules.helper_funcs.string_handling import escape_invalid_curly_brackets
			...
			if MESSAGE_DUMP:
				try:
					...
					if excp.message == "Message to forward not found":
						send_message(update.effective_message, tl(update.effective_message, "This message seems to have been lost - I'll remove it from your notes list."))
				...
				try:
					...
					if excp.message == "Message to forward not found":
						send_message(update.effective_message, tl(update.effective_message, "Looks like the original sender of this note has deleted their message - sorry! Get your bot admin to start using a message dump to avoid this. I'll remove this note from your saved notes."))
						sql.rm_note(chat_id, notename)
					else:
						raise
		else:
			...
			try:
				...
				if note.msgtype in (sql.Types.BUTTON_TEXT, sql.Types.TEXT):
					try:
						...
						send_message(update.effective_message, tl(update.effective_message, "Call me at PM first to get this note."), parse_mode="markdown")
						pass
				else:
					try:
						if is_delete:
				...
				if excp.message == "Entity_mention_user_invalid":
					send_message(update.effective_message, tl(update.effective_message, "Looks like you tried to mention someone I've never seen before. If you really want to mention them, forward one of their messages to me, and I'll be able to tag them!"))
				elif FILE_MATCHER.match(note.value):
					send_message(update.effective_message, tl(update.effective_message, "This note was an incorrectly imported file from another bot - I can't use it. If you really need it, you'll have to save it again. In the meantime, I'll remove it from your notes list."))
					sql.rm_note(chat_id, notename)
				else:
...
def save(update, context):
	...
	try:
		...
		if conn:
			savedtext = tl(update.effective_message, "Ok, saved note <code>{note_name}</code> in <b>{chat_name}</b>.").format(note_name=note_name, chat_name=chat_name)
		else:
			savedtext = tl(update.effective_message,"Ok, saved note <code>{note_name}</code>.").format(note_name=note_name)
		send_message(update.effective_message, savedtext, parse_mode=ParseMode.HTML)

	#if msg.reply_to_message and msg.reply_to_message.from_user.is_bot:
	#    if text:
	#        send_message(update.effective_message, "Looks like you are trying to save a message from the bot. Unfortunately,"
    #                        "bots can't forward bot messages, so I can't save the right message."
    #                        "\nI will keep all the text I can, but if you want more,"
    #                        "You have to do it forward the message yourself, then save it.")
	#    else:
	#        send_message(update.effective_message, "Bots are somewhat deformed by telegrams, making it difficult to interact with"
    #                        "Another bot, so I can't save this message."
    #                        "Do you mind continuing and"
    #                        "then saved the new message? Thank you! ☺️")
	#    return
	...
	if len(args) >= 1:
		...
		if len(catatan) >= 1 and len(catatangagal) == 0:
			...
			try:
				...
				if conn:
					rtext = tl(update.effective_message, "Note in <b>{chat_name}</b> for <code>{note_name}</code> deleted 😁").format(chat_name=chat_name, note_name=", ".join(catatan))
				else:
...
def list_notes(update, context):
	...
	if conn:
		chat_id = conn
		chat_name = dispatcher.bot.getChat(conn).title
		msg = tl(update.effective_message, "*Notes in {}:*\n").format(chat_name)
	else:
		chat_id = update.effective_chat.id
		if chat.type == "private":
			chat_name = ""
	...
	if msg == tl(update.effective_message, "*Notes in {}:*\n").format(chat_name) or msg == tl(update.effective_message, "*Local notes:*\n"):
		...
		try:
			...
			if chat.type == "private":
				...
				chat_name = chat.title
				msg = tl(update.effective_message, "<b>Notes in {}:</b>\n").format(chat_name)
			for note in note_list:
				note_name = " - <code>{}</code>\n".format(note.name)
				if len(msg) + len(note_name) > MAX_MESSAGE_LENGTH:
					send_message(update.effective_message, msg, parse_mode=ParseMode.MARKDOWN)
	...
	if failures:
		with BytesIO(str.encode("\n".join(failures))) as output:
			...
			dispatcher.bot.send_document(chat_id, document=output, filename="failed_imports.txt",
										 caption=tl(update.effective_message, "These files/photos failed to import due to originating from another bot. This is a telegram API restriction, and can't be avoided. Sorry for the inconvenience!"))
