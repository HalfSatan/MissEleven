...
from eleven import dispatcher, spamcheck, LOGGER
from eleven.modules.disable import DisableAbleCommandHandler
from eleven.modules.helper_funcs.chat_status import user_admin_no_reply, user_admin

from eleven.modules.sql import languages_sql as sql
...
for x in os.listdir('eleven/modules/langs'):
	if os.path.isdir('eleven/modules/langs/'+x):
		continue
	x = x.replace('.py', '')
	LOADED_LANGS_ID.append(x)
	imported_langs = importlib.import_module("eleven.modules.langs." + x)
