...
import eleven.modules.sql.notes_sql as sql
from eleven import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, spamcheck, TEMPORARY_DATA
from eleven.__main__ import DATA_IMPORT
from eleven.modules.helper_funcs.chat_status import user_admin
from eleven.modules.helper_funcs.misc import build_keyboard, revert_buttons
from eleven.modules.helper_funcs.msg_types import get_note_type
from eleven.modules.rules import get_rules
from eleven.modules.helper_funcs.string_handling import button_markdown_parser, make_time

# SQL
import eleven.modules.sql.antiflood_sql as antifloodsql
import eleven.modules.sql.blacklist_sql as blacklistsql
import eleven.modules.sql.blsticker_sql as blackliststksql
from eleven.modules.sql import disable_sql as disabledsql
from eleven.modules.sql import cust_filters_sql as filtersql
from eleven.modules.sql import languages_sql as langsql
import eleven.modules.sql.locks_sql as locksql
from eleven.modules.locks import LOCK_TYPES
from eleven.modules.sql import notes_sql as notesql
from eleven.modules.sql import reporting_sql as reportsql
import eleven.modules.sql.rules_sql as rulessql
from eleven.modules.sql import warns_sql as warnssql
import eleven.modules.sql.welcome_sql as welcsql

from eleven.modules.connection import connected

from eleven.modules.helper_funcs.msg_types import Types
...
def import_data(update, context):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			# If backup is from Eleven
			if data.get('bot_base') == "Eleven":
				imp_antiflood = False
				imp_blacklist = False
				...
				if imp_report:
					text += tl(update.effective_message, "- Reporting setting\n")
				if imp_rules:
					text += tl(update.effective_message, "- Rules group\n")
				if imp_lang:
					text += tl(update.effective_message, "- Language settings\n")
				if imp_warn:
					text += tl(update.effective_message, "- Warn settings\n")
				if imp_warn_chat:
					text += tl(update.effective_message, "- {} user warning \n").format(imp_warn_chat)
				if imp_warn_filter:
					text += tl(update.effective_message, "- {} warning filter \n").format(imp_warn_filter)
				try:
				...
				if NOT_IMPORTED_INT:
					...
					f.close()
					context.bot.sendDocument(chat_id, document=open('{}-notimported.txt'.format(chat_id), 'rb'), caption=tl(update.effective_message, "*Data which can't be imported*"), timeout=360, parse_mode=ParseMode.MARKDOWN)
					os.remove("{}-notimported.txt".format(chat_id))
				return
		except Exception as err:
			send_message(update.effective_message, tl(update.effective_message, "An exception occured while restoring your data from Eleven backup!\nJoin our [Group support](https://t.me/MissElevenSupport) for reporting and troubleshooting this problem!\n\nThank you""An exception occured while restoring your data from Eleven backup!\nJoin our [Group support](https://t.me/MissElevenSupport) for reporting and troubleshooting this problem!\n\nThank you"), parse_mode="markdown")
			LOGGER.exception("An error when importing from Eleven base!")
			return
		...
		try:
			...
			send_message(update.effective_message, tl(update.effective_message, "An exception occured while restoring your data from Rose backup!\nJoin our [Group support](https://t.me/MissElevenSupport) for reporting and troubleshooting this problem!\n\nThank you"), parse_mode="markdown")
			LOGGER.exception("An error when importing from Rose base!")
			return

		# only import one group
		if len(data) > 1 and str(chat_id) not in data:
			send_message(update.effective_message, tl(update.effective_message, "Theres more than one group here in this file, and none have the same chat id as this group - how do I choose what to import?"))
			return
		...
		try:
			if data.get(str(chat_id)) == None:
				if conn:
					text = tl(update.effective_message, "Backup comes from another chat, I can't return another chat to chat *{}*").format(chat_name)
				else:
					text = tl(update.effective_message, "Backup comes from another chat, I can't return another chat to this chat")
				return send_message(update.effective_message, text, parse_mode="markdown")
		except:
			return send_message(update.effective_message, tl(update.effective_message, "An exception occured while restoring your data, please report to my creator for solve this problem to make me better! Thanks! 🙂"))
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat_id)]['bot']):
				return send_message(update.effective_message, tl(update.effective_message, "Backup comes from other bots, documents, photos, videos, audio, sounds will not "
		...
		try:
			...
			send_message(update.effective_message, tl(update.effective_message, "An exception occured while restoring your data. The process may not be complete. If "
...
def export_data(update, context):
	...
	if cek.get('status'):
		if jam <= int(cek.get('value')):
			...
			send_message(update.effective_message, tl(update.effective_message, "You can back up your data once in 12 hours!\n[This person](tg://user?Id={}) has backed up your data\nYou can back up your data on`{}`").format(cek.get('user'), waktu), parse_mode=ParseMode.MARKDOWN)
			return
		else:
			if user.id != OWNER_ID:
