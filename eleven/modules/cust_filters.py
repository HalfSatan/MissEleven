...
from eleven.modules.helper_funcs.misc import build_keyboard_parser
from eleven.modules.helper_funcs.msg_types import get_filter_type
from eleven.modules.helper_funcs.string_handling import split_quotes, button_markdown_parser, escape_invalid_curly_brackets
from eleven.modules.sql import cust_filters_sql as sql
...
def list_handlers(update, context):
	...
	if not conn == False:
		...
		if chat.type == "private":
			...
			filter_list = tl(update.effective_message, "*Filters in {}*:\n")
...
def stop_filter(update, context):
	...
	for keyword in chat_filters:
		if keyword == args[1]:
			...
			send_message(update.effective_message, tl(update.effective_message, "Yep, I'll stop replying than in *{}*.").format(chat_name), parse_mode=telegram.ParseMode.MARKDOWN)
			raise DispatcherHandlerStop
				...
				if filt.is_sticker:
					...
					try:
						...
						if excp.message == "Unsupported url protocol":
							try:
								send_message(update.effective_message, tl(update.effective_message, "You seem to be trying to use an unsupported url protocol. Telegram doesn't support buttons for some protocols, such as tg://. Please try again"))
							except BadRequest as excp:
...
def get_exception(excp, filt, chat):
	if excp.message == "Unsupported url protocol":
		...
		LOGGER.warning("Message %s could not be parsed", str(filt.reply))
		LOGGER.exception("Could not parse filter %s in chat %s", str(filt.keyword), str(chat.id))
		return "This note could not be sent, as it is incorrectly formatted."
