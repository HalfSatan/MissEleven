...
from eleven import dispatcher, LOGGER, spamcheck, OWNER_ID, SUDO_USERS
from eleven.modules.helper_funcs.chat_status import user_not_admin, user_admin
from eleven.modules.log_channel import loggable
from eleven.modules.sql import reporting_sql as sql
	...
	if chat.type == chat.PRIVATE:
		if len(args) >= 1:
			if args[0] in ("yes", "on"):
				...
				send_message(update.effective_message, tl(update.effective_message, "Turned on reporting! You'll be notified whenever anyone reports something."))

			elif args[0] in ("no", "off"):
				sql.set_user_setting(chat.id, False)
				send_message(update.effective_message, tl(update.effective_message, "Turned off reporting! You wont get any reports."))
		else:
			send_message(update.effective_message, tl(update.effective_message, "Your current report preference is: `{}`").format(sql.user_should_report(chat.id)),
		...
		if len(args) >= 1:
			if args[0] in ("yes", "on"):
				...
				send_message(update.effective_message, tl(update.effective_message, "Turned on reporting! Admins who have turned on reports will be notified when /report or @admin are called."))

			elif args[0] in ("no", "off"):
				sql.set_chat_setting(chat.id, False)
				send_message(update.effective_message, tl(update.effective_message, "Turned off reporting! No admins will be notified on /report or @admin."))
		else:
			send_message(update.effective_message, tl(update.effective_message, "This chat's current setting is: `{}`").format(sql.chat_should_report(chat.id)),
						   parse_mode=ParseMode.MARKDOWN)
...
def report(update, context) -> str:
	...
	if chat and message.reply_to_message and sql.chat_should_report(chat.id):
		...
		if chat.username and chat.type == Chat.SUPERGROUP:
			...
			msg = tl(update.effective_message, "{} call the admin at \"{}\"!").format(mention_html(user.id, user.first_name),
		...
		keyboard = [
			  [InlineKeyboardButton(tl(update.effective_message, "⚠️ Message reported"), url=chatlink)],
			  [InlineKeyboardButton(tl(update.effective_message, "⚠️ Kick"), callback_data="rp_{}=1={}".format(chat.id, reported_user.id)),
			  InlineKeyboardButton(tl(update.effective_message, "⛔️ Banned"), callback_data="rp_{}=2={}".format(chat.id, reported_user.id))],
			  [InlineKeyboardButton(tl(update.effective_message, "Delete message"), callback_data="rp_{}=3={}".format(chat.id, message.reply_to_message.message_id))],
			  [InlineKeyboardButton(tl(update.effective_message, "Close button"), callback_data="rp_{}=4={}".format(chat.id, reported_user.id))]
			]
		reply_markup = InlineKeyboardMarkup(keyboard)

		should_forward = True
		context.bot.send_message(chat.id, tl(update.effective_message, "<i>⚠️ Message has been reported to all admins!</i>"), parse_mode=ParseMode.HTML, reply_to_message_id=message.message_id)
...
def report_alt(update, context) -> str:
	...
	if chat and message.reply_to_message and sql.chat_should_report(chat.id):
		...
		context.bot.send_message(chat.id, tl(update.effective_message, "⚠️ {} <b>has been reported to the admin!</b>{}").format(
...
def button(bot, update):
	...
	if msg == None or userinfo == None or key == None:
		query.message.edit_text(tl(update.effective_message, "Session is time out!"))
		return

	if splitter[1] == "1":
		keyboard = [
			[InlineKeyboardButton(tl(update.effective_message, "Yes"), callback_data="ak_1+y|{}={}".format(report_chat, report_target)),
			InlineKeyboardButton(tl(update.effective_message, "No"), callback_data="ak_1+n|{}={}".format(report_chat, report_target))]
		]
		reply_markup = InlineKeyboardMarkup(keyboard)
		...
		keyboard = [
			[InlineKeyboardButton(tl(update.effective_message, "Yes"), callback_data="ak_2+y|{}={}".format(report_chat, report_target)),
			InlineKeyboardButton(tl(update.effective_message, "No"), callback_data="ak_2+n|{}={}".format(report_chat, report_target))]
		]
		reply_markup = InlineKeyboardMarkup(keyboard)
		context.bot.edit_message_text(text=msg + tl(update.effective_message, "\n\nAre you sure you want to be banned {}?").format(userinfo.get('name')),
						  chat_id=query.message.chat_id,
						  message_id=query.message.message_id, parse_mode=ParseMode.HTML,
						  reply_markup=reply_markup)
	elif splitter[1] == "3":
		keyboard = [
			[InlineKeyboardButton(tl(update.effective_message, "Yes"), callback_data="ak_3+y|{}={}".format(report_chat, report_target)),
			InlineKeyboardButton(tl(update.effective_message, "No"), callback_data="ak_3+n|{}={}".format(report_chat, report_target))]
		]
		reply_markup = InlineKeyboardMarkup(keyboard)
		context.bot.edit_message_text(text=msg + tl(update.effective_message, "\n\nDelete the message?"),
						  chat_id=query.message.chat_id,
	...
	if isyes == "y":
		...
		if splitter[0] == "1":
			try:
				...
				context.bot.sendMessage(report_chat, text=tl(update.effective_message, "{} has been kicked! \nBy: {}").format(\
			...
			try:
				...
				context.bot.sendMessage(report_chat, text=tl(update.effective_message, "{} has been banned! \nBy: {}").format(\
					mention_markdown(userinfo['id'], userinfo['name']), mention_markdown(chat.id, chat.first_name)), \
					parse_mode=ParseMode.MARKDOWN)
...
def user_protection_checker(bot, user_id):
	if not user_id:
		return False, tl(update.effective_message, "You don't seem to be referring to users.")
...
def __chat_settings__(chat_id, user_id):
	return tl(user_id, "This chat is setup to send user reports to admins, via /report and @admin: `{}`").format(
		sql.chat_should_report(chat_id))


def __user_settings__(user_id):
	return tl(user_id, "You receive reports from chats you're admin in: `{}`.\nToggle this with /reports in PM.").format(
