...
from eleven import spamcheck, OWNER_ID
from eleven.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from eleven import dispatcher, LOGGER
    from eleven.modules.helper_funcs.chat_status import user_admin
    from eleven.modules.sql import log_channel_sql as sql
    ...
    def logging(update, context):
        ...
        if log_channel:
            ...
            send_message(update.effective_message, 
                tl(update.effective_message, "This group has all it's logs sent to: {} (`{}`)").format(escape_markdown(log_channel_info.title),
                                                                         log_channel),
                parse_mode=ParseMode.MARKDOWN)

        else:
            send_message(update.effective_message, tl(update.effective_message, "No log channel has been set for this group!"))
    ...
    def setlog(update, context):
        ...
        if chat.type == chat.CHANNEL:
            send_message(update.effective_message, tl(update.effective_message, "Now, forward the /setlog to the group you want to tie this channel to!"))
            ...
            try:
                context.bot.send_message(message.forward_from_chat.id,
                             tl(update.effective_message, "This channel has been set as the log channel for {}.").format(
                                 chat.title or chat.first_name))
            except Unauthorized as excp:
                if excp.message == "Forbidden: bot is not a member of the channel chat":
                    context.bot.send_message(chat.id, tl(update.effective_message, "Failed to set the log channel!\nI might not be the admin on the channel."))
                    sql.stop_chat_logging(chat.id)
                    return
                else:
                    LOGGER.exception("ERROR in setting the log channel.")
                    
            context.bot.send_message(chat.id, tl(update.effective_message, "Successfully set log channel!"))

        else:
            send_message(update.effective_message, tl(update.effective_message, "The steps to set a log channel are:\n - add bot to the desired channel\n - send /setlog to the channel\n - forward the /setlog to the group\n"))
    ...
    def unsetlog(update, context):
        ...
        if log_channel:
            context.bot.send_message(log_channel, tl(update.effective_message, "Channel has been unlinked from {}").format(chat.title))
