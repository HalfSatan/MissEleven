...
from eleven.modules.sql import admin_sql as sql
...
def demote(update, context):
	...
	if not user_member.status == 'administrator':
		send_message(update.effective_message, tl(update.effective_message, "Can't demote what wasn't promoted!"))
		return ""
	...
	try:
		...
		send_message(update.effective_message, tl(update.effective_message, "Could not demote. I might not be admin, or the admin status was appointed by another user, so I can't act upon them!"))
		return ""
...
def adminlist(update, context):
	...
	text = tl(update.effective_message, "Admin in *{}*:").format(update.effective_chat.title or tl(update.effective_message, "current chat"))
...
def permanent_pin_set(update, context):
	...
	if conn:
		...
		if not args:
			...
			text_maker = tl(update.effective_message, "Current permanent pin: `{}`").format(bool(int(get_permapin)))
			if get_permapin:
				if chat.username:
		...
		if prev_message == "off":
			...
			send_message(update.effective_message, tl(update.effective_message, "Permanent pin has been disabled!"))
			return
		if "/" in prev_message:
			prev_message = prev_message.split("/")[-1]
	else:
	...
	if prev_message and is_group:
		...
		send_message(update.effective_message, tl(update.effective_message, "Successfully set permanent pin!"))
		return "<b>{}:</b>" \
			   "\n#PERMANENT_PIN" \
			   "\n<b>Admin:</b> {}".format(html.escape(chat.title), mention_html(user.id, user.first_name))
