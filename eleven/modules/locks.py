...
import eleven.modules.sql.locks_sql as sql
from eleven import dispatcher, spamcheck, SUDO_USERS, LOGGER
from eleven.modules.disable import DisableAbleCommandHandler
from eleven.modules.helper_funcs.chat_status import can_delete, is_user_admin, user_not_admin, user_admin, \
	bot_can_delete, is_bot_admin
from eleven.modules.helper_funcs.filters import CustomFilters
from eleven.modules.log_channel import loggable
from eleven.modules.sql import users_sql
...
def locktypes(update, context):
	...
	perm.sort()
	send_message(update.effective_message, "\n - ".join([tl(update.effective_message, "Locks: ")] + locklist) + "\n\n" + "\n - ".join([tl(update.effective_message, "Permissions: ")] + perm), parse_mode="markdown")
		...
		if len(args) >= 1:
			...
			if ltype in LOCK_TYPES:
				...
				if conn:
					...
					text = tl(update.effective_message, "Locked *{}* messages for all non-admins at *{}*!").format(ltype, chat_name)
				else:
					...
					text = tl(update.effective_message, "Locked *{}* messages for all non-admins!").format(ltype)
				...
				if conn:
					...
					chat_id = conn
					chat_name = chat.title
					text = tl(update.effective_message, "Permission *{}* was locked for all non-admins at *{}*!").format(ltype, chat_name)
				else:
					if update.effective_message.chat.type == "private":
						send_message(update.effective_message, tl(update.effective_message, "You can do this command in groups, not PM"))
						return ""
					...
					chat_id = update.effective_chat.id
					chat_name = update.effective_message.chat.title
					text = tl(update.effective_message, "Permission *{}* was locked for all non-admins!").format(ltype)
				...
				send_message(update.effective_message, tl(update.effective_message, "What are you trying to lock...? Try /locktypes for the list of lockables"))
		else:
		   send_message(update.effective_message, tl(update.effective_message, "What do you want to lock...?"))

	else:
		send_message(update.effective_message, tl(update.effective_message, "I'm not an administrator, or haven't got delete rights."))
		...
		if len(args) >= 1:
			...
			if ltype in LOCK_TYPES:
				...
				if conn:
					...
					chat_id = conn
					chat_name = chat.title
					text = tl(update.effective_message, "Unlocked {} for everyone at *{}*!").format(ltype, chat_name)
				else:
					if update.effective_message.chat.type == "private":
						send_message(update.effective_message, tl(update.effective_message, "You can do this command in groups, not PM"))
						return ""
					...
					chat_id = update.effective_chat.id
					chat_name = update.effective_message.chat.title
					text = tl(update.effective_message, "Unlocked {} for everyone!").format(ltype)
				...
				if conn:
					...
					chat_id = conn
					chat_name = chat.title
					text = tl(update.effective_message, "Permission *{}* was unlocked for all non-admins at *{}*!").format(ltype, chat_name)
				else:
					if update.effective_message.chat.type == "private":
						send_message(update.effective_message, tl(update.effective_message, "You can do this command in groups, not PM"))
						return ""
					...
					chat_id = update.effective_chat.id
					chat_name = update.effective_message.chat.title
					text = tl(update.effective_message, "Permission *{}* was unlocked for all non-admins!").format(ltype)
			...
			send_message(update.effective_message, tl(update.effective_message, "What are you trying to unlock...?"))
	...
	for lockable, filter in LOCK_TYPES.items():
			...
			if sql.is_locked(chat.id, lockable) and can_delete(chat, context.bot.id):
				if message.caption:
					...
					if 'ARABIC' in check:
						...
						if getconf:
							warn(update.effective_user, chat, tl(update.effective_message, "Send 'RTL Text' which currently locked"), message, update.effective_user, conn=False)
						break
				if message.text:
		...
		if lockable == "button":
			if sql.is_locked(chat.id, lockable) and can_delete(chat, context.bot.id):
				if message.reply_markup and message.reply_markup.inline_keyboard:
					...
					getconf = sql.get_lockconf(chat.id)
					if getconf:
						warn(update.effective_user, chat, tl(update.effective_message, "Send 'Button Message' which currently locked"), message, update.effective_user, conn=False)
					break
			continue
		if filter(update) and sql.is_locked(chat.id, lockable) and can_delete(chat, context.bot.id):
			if lockable == "bots":
				...
				for new_mem in new_members:
					if new_mem.is_bot:
						if not is_bot_admin(chat, context.bot.id):
							send_message(update.effective_message, tl(update.effective_message, "I see a bot, and I've been told to stop them joining... but I'm not admin!"))
							return

						chat.kick_member(new_mem.id)
						send_message(update.effective_message, tl(update.effective_message, "Only admins are allowed to add bots to this chat! Get outta here."))
						getconf = sql.get_lockconf(chat.id)
						if getconf:
							warn(update.effective_user, chat, tl(update.effective_message, "Invite 'Bot' which currently locked"), message, update.effective_user, conn=False)
						break
			else:
				try:
					message.delete()
				...
				getconf = sql.get_lockconf(chat.id)
				if getconf:
					warn(update.effective_user, chat, tl(update.effective_message, "Send '{}' which currently locked").format(lockable), message, update.effective_user, conn=False)
...
def build_lock_message(chat_id):
	...
	permslist = []
	if locks:
		res += "*" + tl(chat_id, "These are the locks in this chat:") + "*"
		if locks:
	...
	if locklist:
		...
		for x in locklist:
			res += "\n - {}".format(x)
	res += "\n\n*" + tl(chat_id, "These are permissions in this chat:") + "*"
	for x in permslist:
		res += "\n - {}".format(x)
	return res
...
def list_locks(update, context):
	...
	if conn:
		res = res.replace(tl(update.effective_message, 'this chat'), '*{}*'.format(chat_name))
...
def lock_warns(update, context):
	...
	if args:
		if args[0] == "on" or args[0] == "yes":
			...
			try:
				send_message(update.effective_message, tl(update.effective_message, "I *will warn* user if send any message/media which currently locked"), parse_mode="markdown")
			except BadRequest:
				send_message(update.effective_message, tl(update.effective_message, "I *will warn* user if send any message/media which currently locked", parse_mode="markdown", quote=False))
		elif args[0] == "off" or args[0] == "no":
			sql.set_lockconf(chat_id, False)
			try:
				send_message(update.effective_message, tl(update.effective_message, "I *will not warn* user if send any message/media which currently locked"), parse_mode="markdown")
			except BadRequest:
				send_message(update.effective_message, tl(update.effective_message, "I *will not warn* user if send any message/media which currently locked", parse_mode="markdown", quote=False))
		else:
			try:
				send_message(update.effective_message, tl(update.effective_message, "I only understand 'on/yes' or 'off/no' only!"), parse_mode="markdown")
			except BadRequest:
				send_message(update.effective_message, tl(update.effective_message, "I only understand 'on/yes' or 'off/no' only!"), parse_mode="markdown", quote=False)
	else:
		getconf = sql.get_lockconf(chat_id)
		if getconf:
			try:
				send_message(update.effective_message, tl(update.effective_message, "Currently I *will warn* user if send any message/media which currently locked"), parse_mode="markdown")
			except BadRequest:
				send_message(update.effective_message, tl(update.effective_message, "Currently I *will warn* user if send any message/media which currently locked"), parse_mode="markdown", quote=False)
		else:
			try:
				send_message(update.effective_message, tl(update.effective_message, "Currently I *will not warn* user if send any message/media which currently locked"), parse_mode="markdown")
			except BadRequest:
				send_message(update.effective_message, tl(update.effective_message, "Currently I *will not warn* user if send any message/media which currently locked"), parse_mode="markdown", quote=False)
