...
import eleven.modules.sql.connection_sql as sql
from eleven import dispatcher, LOGGER, SUDO_USERS, spamcheck
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no" or var == "tidak"):
                ...
                send_message(update.effective_message, languages.tl(update.effective_message, "Please enter `yes` or `no`!"), parse_mode=ParseMode.MARKDOWN)
        else:
...
def connect_chat(update, context):
    ...
    if update.effective_chat.type == 'private':
        if args and len(args) >= 1:
            ...
            if gethistory:
                buttons = [InlineKeyboardButton(text=languages.tl(update.effective_message, "❎ Close button"), callback_data="connect_close"), InlineKeyboardButton(text=languages.tl(update.effective_message, "🧹 Clear history"), callback_data="connect_clear")]
            else:
                buttons = []
            ...
            if conn:
                ...
                buttons.append(InlineKeyboardButton(text=languages.tl(update.effective_message, "🔌 Disconnect"), callback_data="connect_disconnect"))
            else:
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            if connection_status:
                ...
                send_message(update.effective_message, languages.tl(update.effective_message, "Successfully connected to *{}*.").format(chat_name), parse_mode=ParseMode.MARKDOWN)
                try:
                    sql.add_history_conn(user.id, str(chat.id), chat_name)
                    context.bot.send_message(update.effective_message.from_user.id, languages.tl(update.effective_message, "You have connected with *{}*. Use /connection for see current available commands.").format(chat_name), parse_mode="markdown")
                except BadRequest:
                    pass
...
def connect_button(update, context) -> str:
    ...
    if connect_match:
        ...
        if (isadmin) or (isallow and ismember) or (user.id in SUDO_USERS):
            ...
            context.bot.answer_callback_query(query.id, languages.tl(update.effective_message, "Connection to this chat is not allowed!"), show_alert=True)
    elif disconnect_match:
        disconnection_status = sql.disconnect(query.from_user.id)
        if disconnection_status:
           sql.disconnected_chat = query.message.edit_text(languages.tl(update.effective_message, "Disconnected from chat!"))
        else:
           context.bot.answer_callback_query(query.id, languages.tl(update.effective_message, "You're not connected!"), show_alert=True)
    elif clear_match:
        sql.clear_history_conn(query.from_user.id)
        query.message.edit_text(languages.tl(update.effective_message, "History connected has been cleared!"))
    elif connect_close:
